...
#Filter
원본 = pd.read_excel(data_URL)
원본['날짜'] = pd.to_datetime(원본['날짜'])

if 'chosen_year_option' not in st.session_state:
    st.session_state.chosen_year_option = '전체'
...
if view_mode == '직관':
    전체 = 원본[원본['장르'].isin(['연극', '뮤지컬', '기타'])].copy()
elif view_mode == '집관':
    전체 = 원본[원본['장르'].str.contains('중계|DVD', na=False)].copy()
else:
    전체 = 원본.copy()
...
if st.session_state.chosen_year_option not in display_list:
    st.session_state.chosen_year_option = '전체'

with col2:
    default_index = display_list.index(st.session_state.chosen_year_option)
    ...
    st.session_state.chosen_year_option = selected_option
...
if is_all:
    ...
    df_year_before = 전체[전체['날짜'].dt.year == (selected_year - 1)].copy()
...
유효장르 = [f"{장르} {count}편" for 장르, count in 장르극.items() if count > 0]
유효장르_text = ", ".join(유효장르)

극_text = f"<p style='font-size: 17px;'>{'&nbsp;' * 2}: {유효장르_text}</p>"
